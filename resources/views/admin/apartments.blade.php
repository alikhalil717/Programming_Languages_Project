...
@section('content')
    <div x-data>
        <h1 class="text-3xl font-bold mb-6" x-text="language === 'ar' ? 'إدارة الشقق' : 'Apartments Management'">Apartments Management</h1>

        <! Stats Cards>
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div class="bg-white p-4 rounded-xl shadow border-t-4 border-indigo-500">
                <p class="text-sm text-gray-500" x-text="language === 'ar' ? 'إجمالي الشقق' : 'Total Apartments'">Total Apartments</p>
                <p id="total-apartments" class="text-2xl font-bold">0</p>
            </div>
            <div class="bg-white p-4 rounded-xl shadow border-t-4 border-green-500">
                <p class="text-sm text-gray-500" x-text="language === 'ar' ? 'الشقق النشطة' : 'Active Apartments'">Active Apartments</p>
                <p id="active-apartments" class="text-2xl font-bold">0</p>
            </div>
            <div class="bg-white p-4 rounded-xl shadow border-t-4 border-yellow-500">
                <p class="text-sm text-gray-500" x-text="language === 'ar' ? 'قيد المراجعة' : 'Pending Review'">Pending Review</p>
                <p id="pending-apartments" class="text-2xl font-bold">0</p>
            </div>
            <div class="bg-white p-4 rounded-xl shadow border-t-4 border-red-500">
                <p class="text-sm text-gray-500" x-text="language === 'ar' ? 'مرفوض' : 'Rejected'">Rejected</p>
        ...
        <div class="bg-white p-4 rounded-xl shadow-lg mb-6">
            <div class="flex flex-col md:flex-row gap-4">
                <div class="flex-1">
                    <div class="relative">
                        <input type="text" id="search-input" :placeholder="language === 'ar' ? 'ابحث بالعنوان أو المدينة...' : 'Search by address or city...'"
                ...
                <div class="flex gap-2">
                    <select id="status-filter"
                        ...
                        <option value="approved" x-text="language === 'ar' ? 'معتمد' : 'Approved'">Approved</option>
                        <option value="pending" x-text="language === 'ar' ? 'قيد المراجعة' : 'Pending Review'">Pending Review</option>
                        <option value="rejected" x-text="language === 'ar' ? 'مرفوض' : 'Rejected'">Rejected</option>
                    </select>

                    <select id="city-filter"
                        class="border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        <option value="" x-text="language === 'ar' ? 'جميع المدن' : 'All Cities'">All Cities</option>
                    </select>
        ...
        <div class="bg-white p-6 rounded-xl shadow-lg">
            <div id="loading" class="text-center py-8">
                ...
                <p class="mt-2 text-gray-500" x-text="language === 'ar' ? 'جاري تحميل الشقق...' : 'Loading apartments...'">Loading apartments...</p>
            </div>

            <div id="error" class="hidden bg-red-50 text-red-700 p-4 rounded-lg mb-4">
                <p id="error-message"></p>
                <button onclick="loadApartments()" class="mt-2 px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 flex items-center">
            ...
            <div id="apartments-content" class="hidden">
                    ...
                    <table class="w-full min-w-full">
                        <thead>
                            <tr class="bg-gray-50">
                                ...
                                <th class="p-3 font-medium text-gray-500" :class="language === 'ar' ? 'text-right' : 'text-left'">
                                    <span x-text="language === 'ar' ? 'المالك' : 'Owner'">Owner</span>
                                </th>
                                <th class="p-3 font-medium text-gray-500" :class="language === 'ar' ? 'text-right' : 'text-left'">
                                    <span x-text="language === 'ar' ? 'المدينة' : 'City'">City</span>
                                </th>
                                <th class="p-3 font-medium text-gray-500" :class="language === 'ar' ? 'text-right' : 'text-left'">
                                    <span x-text="language === 'ar' ? 'السعر' : 'Price'">Price</span>
                                </th>
                                <th class="p-3 font-medium text-gray-500" :class="language === 'ar' ? 'text-right' : 'text-left'">
                                    <span x-text="language === 'ar' ? 'التقييم' : 'Rating'">Rating</span>
                                </th>
                                <th class="p-3 font-medium text-gray-500" :class="language === 'ar' ? 'text-right' : 'text-left'">
                                    <span x-text="language === 'ar' ? 'الحالة' : 'Status'">Status</span>
                                </th>
                                <th class="p-3 font-medium text-gray-500" :class="language === 'ar' ? 'text-right' : 'text-left'">
                                    <span x-text="language === 'ar' ? 'تاريخ الإضافة' : 'Added Date'">Added Date</span>
                                </th>
                                <th class="p-3 font-medium text-gray-500" :class="language === 'ar' ? 'text-right' : 'text-left'">
                ...
                <div id="no-results" class="text-center p-8 hidden">
                    ...
                    <p class="text-gray-500" x-text="language === 'ar' ? 'لا توجد شقق للعرض' : 'No apartments to display'">No apartments to display</p>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white rounded-xl shadow-2xl w-full max-w-4xl mx-4 max-h-[90vh] overflow-y-auto">
                <div class="p-6">
                    <div class="flex justify-between items-center mb-6" :class="language === 'ar' ? 'flex-row-reverse' : ''">
                        <h3 class="text-xl font-bold" x-text="language === 'ar' ? 'تفاصيل الشقة' : 'Apartment Details'">Apartment Details</h3>
    ...
    <script>
        ...
        async function loadCities() {
            try {
                ...
                cityFilter.innerHTML = '<option value="">' + (window.Alpine.$data.language === 'ar' ? 'جميع المدن' : 'All Cities') + '</option>';
                cities.forEach(city => {
        ...
        function displayApartments(response) {
            ...
            response.apartments.forEach(apartment => {
                ...
                <tr class="border-b hover:bg-gray-50">
                    <td class="p-3">
                        <div class="flex items-center" ${language === 'ar' ? 'style="direction: rtl; text-align: right;"' : ''}>
                            <div class="w-12 h-12 rounded-lg overflow-hidden" ${language === 'ar' ? 'style="margin-right: 0.75rem; margin-left: 0;"' : 'style="margin-left: 0.75rem; margin-right: 0;"'}>
                                ${apartment.images && apartment.images[0] && apartment.images[0].image_path ?
                    ...
                    <td class="p-3">
                        <div class="flex items-center" ${language === 'ar' ? 'style="direction: rtl; text-align: right;"' : ''}>
                            <i class="fas fa-map-marker-alt text-gray-400" ${language === 'ar' ? 'style="margin-right: 0.5rem; margin-left: 0;"' : 'style="margin-left: 0.5rem; margin-right: 0;"'}></i>
                            ${apartment.city || 'Not specified'}
                        </div>
                    </td>
                    <td class="p-3 font-semibold">
                        ${apartment.price_per_night || 0}$ / ${language === 'ar' ? 'ليلة' : 'night'}
                    </td>
                    <td class="p-3">
                        <div class="flex items-center" ${language === 'ar' ? 'style="direction: rtl; text-align: right;"' : ''}>
                    ...
                    <td class="p-3">
                        <span class="px-3 py-1 rounded-full text-sm ${getStatusClass(apartment.status)}">
                            ${getStatusText(apartment.status, language)}
                        </span>
                    </td>
                    ...
                    <td class="p-3">
                        <div class="flex gap-2" ${language === 'ar' ? 'style="direction: rtl; text-align: right;"' : ''}>
            ...
            if (response.pagination) {
                ...
                const showingText = language === 'ar'
                    ? `عرض ${response.apartments.length} من ${response.pagination.total || 0} شقة`
                    : `Showing ${response.apartments.length} of ${response.pagination.total || 0} apartments`;
        ...
        async function viewApartmentDetails(apartmentId) {
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                            ${apartment.images && apartment.images.length > 0 ?
                                ...
                                `<div class="col-span-3 bg-gray-100 rounded-lg p-8 text-center">
                                    ...
                                    <p class="text-gray-500">${language === 'ar' ? 'لا توجد صور' : 'No images'}</p>
                                </div>`
                            }
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="bg-gray-50 p-4 rounded-lg" ${language === 'ar' ? 'style="direction: rtl; text-align: right;"' : ''}>
                                <p class="text-sm text-gray-500">${language === 'ar' ? 'العنوان' : 'Address'}</p>
                                <p class="font-medium text-lg">${apartment.title}</p>
                                <p class="text-gray-600">${apartment.address}</p>
                            </div>

                            <div class="bg-gray-50 p-4 rounded-lg" ${language === 'ar' ? 'style="direction: rtl; text-align: right;"' : ''}>
                                <p class="text-sm text-gray-500">${language === 'ar' ? 'المالك' : 'Owner'}</p>
                                <div class="flex items-center mt-1" ${language === 'ar' ? 'style="direction: rtl; text-align: right;"' : ''}>
                                    <div class="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center" ${language === 'ar' ? 'style="margin-right: 0.75rem; margin-left: 0;"' : 'style="margin-left: 0.75rem; margin-right: 0;"'}>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                            <div class="bg-gray-50 p-3 rounded-lg" ${language === 'ar' ? 'style="direction: rtl; text-align: right;"' : ''}>
                                <p class="text-sm text-gray-500">${language === 'ar' ? 'المدينة' : 'City'}</p>
                                <p class="font-medium">${apartment.city}</p>
                            </div>

                            <div class="bg-gray-50 p-3 rounded-lg" ${language === 'ar' ? 'style="direction: rtl; text-align: right;"' : ''}>
                                <p class="text-sm text-gray-500">${language === 'ar' ? 'السعر' : 'Price'}</p>
                                <p class="font-medium text-indigo-600">${apartment.price_per_night}$ / ${language === 'ar' ? 'ليلة' : 'night'}</p>
                            </div>

                            <div class="bg-gray-50 p-3 rounded-lg" ${language === 'ar' ? 'style="direction: rtl; text-align: right;"' : ''}>
                                <p class="text-sm text-gray-500">${language === 'ar' ? 'عدد الغرف' : 'Bedrooms'}</p>
                                <p class="font-medium">${apartment.bedrooms || 1} ${language === 'ar' ? 'غرف' : 'rooms'}</p>
                            </div>

                            <div class="bg-gray-50 p-3 rounded-lg" ${language === 'ar' ? 'style="direction: rtl; text-align: right;"' : ''}>
                                <p class="text-sm text-gray-500">${language === 'ar' ? 'السعة' : 'Capacity'}</p>
                                <p class="font-medium">${apartment.capacity || 2} ${language === 'ar' ? 'أشخاص' : 'persons'}</p>
                            </div>
                        </div>
                        ...
                        <div class="bg-gray-50 p-4 rounded-lg" ${language === 'ar' ? 'style="direction: rtl; text-align: right;"' : ''}>
                            <div class="flex justify-between items-center" ${language === 'ar' ? 'style="direction: rtl; text-align: right; flex-direction: row-reverse;"' : ''}>
                                <div>
                                    <p class="text-sm text-gray-500">${language === 'ar' ? 'حالة الشقة' : 'Apartment Status'}</p>
                                    <p class="font-medium ${getStatusClass(apartment.status)}">
                                        ${getStatusText(apartment.status, language)}
                                    </p>
                                </div>
                                <div>
                                    <p class="text-sm text-gray-500">${language === 'ar' ? 'تاريخ الإضافة' : 'Added Date'}</p>
                                    <p class="font-medium">${formatDate(apartment.created_at)}</p>
                                </div>
                            </div>

                            <div class="mt-4 flex gap-3" ${language === 'ar' ? 'style="direction: rtl; text-align: right; flex-direction: row-reverse;"' : ''}>
                                ${apartment.status === 'pending' ?
                        ...
                        <!-- Description -->
                        <div class="bg-gray-50 p-4 rounded-lg" ${language === 'ar' ? 'style="direction: rtl; text-align: right;"' : ''}>
                            <p class="text-sm text-gray-500 mb-2">${language === 'ar' ? 'الوصف' : 'Description'}</p>
                        ...
                        ${apartment.amenities && apartment.amenities.length > 0 ? `
                            <div class="bg-gray-50 p-4 rounded-lg" ${language === 'ar' ? 'style="direction: rtl; text-align: right;"' : ''}>
                                <p class="text-sm text-gray-500 mb-2">${language === 'ar' ? 'المرافق' : 'Amenities'}</p>
                                <div class="flex flex-wrap gap-2" ${language === 'ar' ? 'style="direction: rtl; text-align: right; flex-direction: row-reverse;"' : ''}>
                                    ${apartment.amenities.map(amenity => `
                                        <span class="px-3 py-1 bg-white rounded-full text-sm">
                                            <i class="fas fa-check text-green-500" ${language === 'ar' ? 'style="margin-right: 0.5rem; margin-left: 0;"' : 'style="margin-left: 0.5rem; margin-right: 0;"'}></i>
                                            ${amenity}
                                        </span>
                                    `).join('')}
                                </div>
            ...
            async function approveApartment(apartmentId) {
                const language = window.Alpine.$data.language || 'en';
                const confirmMsg = language === 'ar' ? 'هل تريد اعتماد هذه الشقة؟' : 'Do you want to approve this apartment?';
                ...
                try {
                    ...
                    if (response && response.success) {
                        showSuccess(language === 'ar' ? 'تم اعتماد الشقة بنجاح' : 'Apartment approved successfully');
                        loadApartments();
                        loadStats();
                    } else {
            ...
            async function rejectApartment(apartmentId) {
                const language = window.Alpine.$data.language || 'en';
                const confirmMsg = language === 'ar' ? 'هل تريد رفض هذه الشقة؟' : 'Do you want to reject this apartment?';
                ...
                try {
                    ...
                    if (response && response.success) {
                        showSuccess(language === 'ar' ? 'تم رفض الشقة بنجاح' : 'Apartment rejected successfully');
                        loadApartments();
                        loadStats();
                    } else {
            ...
            function getStatusText(status, language = 'en') {
                switch (status) {
                    case 'approved': return language === 'ar' ? 'معتمد' : 'Approved';
